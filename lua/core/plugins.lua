...
require("lazy").setup({
	spec = {
		...
		{
			"folke/trouble.nvim",
			-- Плагин должен загрузиться, как только будет вызвана эта команда
			cmd = { "TroubleToggle", "Trouble" },

			-- Убедитесь, что эта зависимость есть, чтобы отображались иконки
			dependencies = { "nvim-tree/nvim-web-devicons" },
			opts = {},

			-- Маппинги, которые используют команду (самый надежный способ)
			keys = {
				-- Основной маппинг: <leader>xx вызывает команду :TroubleToggle
				{ "<leader>xx", "<cmd>TroubleToggle<CR>", desc = "Toggle Trouble list (Diagnostics)" },

				-- Дополнительные маппинги, использующие команду:
				{ "<leader>xw", "<cmd>TroubleToggle workspace_diagnostics<CR>", desc = "Toggle Trouble (Workspace)" },
				{ "<leader>xd", "<cmd>TroubleToggle document_diagnostics<CR>", desc = "Toggle Trouble (File)" },
